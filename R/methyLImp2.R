...
#' @param groups a vector of the same length as the number of samples that identifies 
...
#'
#' @return Either a numeric matrix \eqn{out} with imputed data or 
    ...
    for (i in 1:ngroups) {
        ...
        if (sum(vapply(res, function(x) is.character(x), logical(1))) > 0) {
            ...
            for (i in length(problem_chr)) {
                message("For group " , curr_group, "and chromosome ", problem_chr[i], 
                        ": ", res[problem_chr[i]])
            }
        }
