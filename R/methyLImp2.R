...
#'
#' @param data a numeric data matrix with missing values, 
#' with samples in rows and variables (probes) in columns.
#' @param type a type of data. It is used to provide correct annotation: 
#' 450K, EPIC or user-provided. If the latter is chosen, annotation must be provided in the next argument.
#' @param annotation a data frame, user provided annotation. 
#' Must contain two columns: cpg and chr, and provide a match between CpG sites and chromosomes. 
#' Choose "user" in the previous argument to be able to provide user annotation.
...
methyLImp2 <- function(data,
    ...
    data_chr <- data_chr[names(data_chr)[vapply(data_chr, 
                                                function(x) dim(x)[2], numeric(1)) != 0]]
    nchr <- length(data_chr)
    #reorder chromosomes from biggest to smallest
    chr_order <- names(sort(vapply(data_chr, function(x) dim(x)[2], numeric(1)), 
                            decreasing = TRUE))
    ...
    if (sum(vapply(res, function(x) is.character(x), logical(1))) > 0) {
        ...
        for (i in length(problem_chr)) {
            message("For chromosome ", problem_chr[i], ": ", res[problem_chr[i]])
        }
    }
