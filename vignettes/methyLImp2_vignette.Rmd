...
This version is parallelized over chromosomes and, optionally, in case of large sample size, can use mini-batch approach. In our simulation study this two improvements allowed to reduce running time from more than a day to just half an hour for the EPIC dataset of 456 samples!

In this vignette we will demonstrate the usage of the *methyLImp2* for the imputation of the missing values in the methylation dataset. 
...
```{r construct SE}
data(beta_meta)
beta_SE <- SummarizedExperiment(assays = list(t(beta_with_nas)), 
                                colData = beta_meta)
```
